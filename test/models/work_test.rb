...
describe Work do
  ...
  describe 'top_media' do
    it 'can return the work with the most number of votes' do
      work = Work.top_media
      expect(work.title).must_equal "Book 1"
    end

    it 'will return nil when no works are available to spotlight' do
      Work.find_by(title: "Book 1").delete
      Work.find_by(title: "Book 2").delete
    ...
    it 'will return first media when there is a tie' do
      ...
      expect(Work.top_media.title).must_equal "Book 1"
    end
  end
  ...
  describe 'top_ten' do
    it 'will return an array of 10 elements if there are 10 or more works' do
      10.times do
        Work.create(title: "New Book", category: "book")
      end
